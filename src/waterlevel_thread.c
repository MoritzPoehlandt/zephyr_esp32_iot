...
K_MSGQ_DEFINE(waterlevel_msgq, sizeof(int16_t), MSG_QUEUE_SIZE, 4);

K_MUTEX_DEFINE(level_mutex);
...
static int run_waterlevel() {
	...
	LOG_INF("Waterlevel Threads started ");
	...
	while (1) {
		...
		for (size_t i = 0U; i < ARRAY_SIZE(adc_channels); i++) {
			...
			if (err < 0) {
				...
				k_msgq_put(&waterlevel_msgq, &val_mv, K_NO_WAIT);
			}
		}
